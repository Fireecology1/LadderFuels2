...
f15 <- formula(CFI ~ ws + I(FSG^1.5) + SFC3 + ws:MC.FFMC)
f16 <- formula(CFI ~ ws + I(FSG^1.5) + SFC3 + ws:MC.SA)
...
XVM.summary.lin <- data.frame(mod=as.character(modelform)[3], 
                         multval, 
...
final.mod.lin.sa <- glm(f16, data=fd7, family=binomial)
final.mod.lin.ffmc <- glm(f15, data=fd7, family=binomial)

#write.csv(final.mod.lin.sa$coefficients, './tables/final_mod_sa.csv')
#I don't think this works better than the log(SFC) models, unfortunately
