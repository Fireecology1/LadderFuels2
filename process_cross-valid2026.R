...
setwd("C:/Dan/_Remote_projects/LadderFuels2/files/")
...
fd5 <- fd4
fd5$FSG[sharp] <- fd4$FSG[sharp]-2 #IM stands: FSG=CBH-2
fd5$FSG[sharp.sm] <- fd4$FSG[sharp.sm]-1  #SM stands: FSG=CBH-1
fd5$FSG[sharp.th] <- fd4$FSG[sharp.th]-0.25  #TH stands FSG=CBH-0.25 not always used; also tried 0.3
fd5$FSG[icfme] <- filter(fd5, num %in% icfme) %>% 
  mutate(FSG=case_when(
    FSG > 2 ~ FSG-1, #reduced due to dead ladder fuels
    TRUE ~ FSG)) %>% pull(FSG) #leave alone if FSG already <= 2 m
...
XV.sum2.log <- data.frame(model.order2, reorder2, 
