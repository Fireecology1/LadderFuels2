...
been cited become a classic (\> 1600 citations as of early 2026), methods for
quantifying the effects of ladder fuels are still lacking nearly 50 years
later.
...
@andrews2014], or with empirical ROS functions, such as in the FBP System  [@forestrycanadafiredangergroup1992]. Surprisingly, a simple
...
heights. Empirical data presently allows for calibration of the theoretical and conceptual linkages between surface and crown fire processes.
...
based on a rearrangement of Van Wagner's (1977) equation; 2) calibrate and evaluate the theoretical equation with field data; and 3) incorporate the ladder fuel function into an empirical operational crown fire model for use with
measured or estimated fuel structure.
...
continuous with the surface fuelbed and ignited by a proximate source (i.e.
...
[eq. 2], where $H$ is the heat of combustion, $SFC$ is surface fuel consumption,
...
$FC_L$ positioned at certain height above ground $C_L$ (such as the centroid of
...
[eq. 6]. Using Equation 6, it becomes possible to estimate the contribution
...
ladder fuel scaling relationship and calibrate the value of the multiplier M: 1) an expert opinion assessment, and 2) a crown fire threshold model between three treatment types. The relationship was further evaluated within the context of an empirical crown fire model using Canadian experimental fire data, by a sensitivity analysis of model performance with and without ladder fuels and with different values for M. The final step leads to a new concept of crown fire, where flaming transitions between canopy fuel strata can be tested and defined individually. 
All of these relate to stand level estimates of fuel structure using mean values, which is simple and operationally practical, but may mask significant impacts of within-stand heterogeneity and spatial pattern of fuel arrangement (e.g. clumping, proximity to overstory trees).

### Study area: Sharpsand Creek, Ontario

The validation made use of data from the fire experiments at
...
regenerated jack pine following a 1948 wildfire, with dense stocking of a single live cohort along with abundant standing dead stems forming volatile ladder fuels [@walker1975; @stocks1973]. Results from the
...
During the course of the early experiments, it was discovered that 6 of the
plots had been hand-thinned (TH) in 1960, removing most of the regenerating 
...
as the IM stands assigned to the C-4 fuel type [@degroot2022]. In later studies, the ladder fuel influence in IM stands was estimated by some authors to act similarly to a reduction of the LCBH; this was based on the logic of reduced distance between surface fuel and continuous canopy fuel caused by the standing dead ladder fuels. This modified LCBH measure was termed the Fuel Strata Gap, equal to 2 m [@cruz2004; @cruz2006; @perrakis2023].
...
experiments (1974-1981; IM and TH stands: Stocks 1987; Cruz 1999), fire type was
identified as 'surface' or 'crown'. In the SM experiments, however,
...
threshold (i.e., primarily surface fires but with \~5-15% crown fraction burned). Following discussions with experimental leaders (BJ Stocks, pers. comm. 2020), the two fires described as 'some torching' were assumed to defined the crowning threshold: fire #16 (ISI=8.1, ROS=3.5 m/min) was considered to be a surface fire observation, while fire #18 (ISI=9.4, ROS=5.1 m/min) was considered a marginal passive crown fire. 
...
$$C_L=CBH+\frac {height-CBH}{2}$$, 
[eq. 7] [@perrakis2023] at the stand level, where
...
The first method involved expert opinion from previous studies. As noted above, Cruz et al. [-@cruz2004] noted that the abundant ladder fuels characterizing the Sharpsand Creek, ON IM stands, in the form of standing dead trees (including dead branches, bark flakes, and fine stemwood) amounted to a reduced FSG of 2 m,
...
index (ISI) from the FWI System [@vanwagner1987]; we also tested for thresholds using a stand-adjusted version of the ISI (ISI~SA~), which was simply the ISI
equation's f(F) function fitted to the MC~SA~ [@perrakis2026].
...
[eq. 8]. Starting with the approximate LCBH values originally reported for the
Sharpsand IM stands (*z_1*= 4 m), this value was reduced by 2 m to produce the
FSG estimate (*z_2*=2 m). The mean estimated *FC_L* of the IM plots was 0.157 kg m^[-2], and the mean SFC was 1.405. Therefore, we solve eq. 8 for our first estimate for M:

$$\begin{aligned}
M
&=\frac {1.405 \cdot (4-2)^{1.5}} {0.157 \cdot 2^{1.5}} \\
M&=8.95 \end{aligned}$$ [7]. 
The same calculation was also completed with more precise values derived from original stand data [@walker1975]. Detailed plot-level analyses [@degroot2022; @perrakis2023] produced mean treatment-level LCBH values of 4.36 m in the IM plots. Solving eq. 8 with *z_1*=4.36 and *z_2*=2.36 (following the assumption of $FSG=z_2=z_1-2$) produces a slightly lower estimate of M=6.990. 

#### 3.2 Crowning thresholds and critical ROS, Sharpsand Creek

The second multiplier estimation method involted the logistic regression between treatments. The full dataset consisted of 27 experimental fires: 14 IM, 5 TH, and 8 SM. The logistic regressions for ISI and ISI_sa showed both ISI/ISI_sa and treatment as significant predictors (⍺=0.05) of crown fire occurrence.   
...
Solving for M also required a surface ROS estimate associated with the crowning
thresholds in both stand types. We used an ensemble of 5 different recommended sROS models for hemi-boreal stands, using the ISI, ISI_sa, or individual moisture and wind speed variables [@perrakis2026]: M12 (ISI), M13 (ISI_sa), M16, M17, M18 and M20%, the latter being the 'mental arithmetic' model that estimates sROS in conifer stands as 20 % of the open wind speed. While these sROS models suffer somewhat from sparse observations for ISI > \~12, typical crown fire conditions, they are quite credible under conditions approximating our crowning thresholds of ISI/ ISI_sa of \~5-10. Approximate surface fire spread rates of 0.7 – 2.5 m min^-1^ are expected.

##### 3.2.2 Byram's intensity and estimating *M*

We can now use our scaled ladder fuel equation (eq.6) along with
estimated threshold surface ROS and Byram's fire intensity equation [@byram1959] once again in order to calculate the most likely value for $M$. This calculation assumes that equivalent critical surface intensity values ($I_0$) are required for crowning in the control and thinned stands, both with LCBH of about 4.4 m.
...
Based on eq. 6, $FC_{SE}$ is calculated using ladder fuel structure and estimated consumption of \< 1 cm dead roundwood. In this case, we calculate ladder fuel height ($C_L$) using an estimated crown ratio of 0.5 and cohort heights from
Walker and Stocks' [-@walker1975] original stand data. Mean dead sapling
height and \< 1 cm dead consumption ($FC_L$ ) in control stands were 3.49
m and 0.159 kg m^-2^, resulting in mean $C_L=2.62$ m. $FC_{SE}$ values (eq. 6), ranged from
...
[eq. 10]. Mean $SFC_{Im}$ was 1.414 kg m^{-2}, while mean $SFC_{Th}$ was 1.124 kg m^{-2}, with the difference being non-significant (t-test: P=0.141). 
...
using the ISI-based sROS model M12 and
crowning thresholds, and 
$$\begin{align*}
...
using the ISI~SA~-based sROS model M13 and the two 
threshold ISI~SA~ values.
...
M20%: Use mean overall FFMC of 90.89 - ws=6.07 (IM), ws=14.88
IM: 1.214
TH: 2.976
M=2.949

m16 (mc, ws): mean FFMC of 90.89
IM: 0.5525988
TH: 3.074282
M=9.272271
...
columns: (CR=0.5) IM thresh, TH thresh; (CR=1) IM thresh, TH thresh
...
#vector of ISI- or wind-based ROS estimates
#m12, m16(mc, ws), m18, m20%
isi.mods <- c('M12', 'M16', 'M18', 'M20%')

#vector of isi.m-based ROS estimates
#m9, m13, m17(mcsa, ws)
isim.mods <- c('M9', 'M13','M17')


#check with m12 to start:
thresh.im <- fun.m12(isi.thresh.im)
thresh.th <- fun.m12(isi.thresh.th)
