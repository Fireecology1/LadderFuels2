...
#get models from sROS project
m9 <- load('./files/sros-m9.rda')
m12 <- load('./files/sros-m12.rda')
m13 <- load('./files/sros-m13.rda')
m14 <- load('./files/sros-m14.rda')
m16 <- load('./files/sros-m16.rda')
m17 <- load('./files/sros-m17.rda')
m18 <- load('./files/sros-m18.rda')
...
sh.ctrl <- filter(sh.full, TRT=='ctrl') %>% 
  mutate(cfc.d=lf.cfl2,
         cfc.d.stem=cfc.d * 10000/s.ha.d,   #kg/m2*10000 m2/ha/s/ha = kg/stem
         c.l= HT.D * 0.75,  #assume CR of 0.5;
         sfc.add=fc.se(zg=FSG, cl=c.l, fcl=cfc.d), #SFC.2=(z.2/z.1)^1.5 * SFC.1
         sfc.add.stem=sfc.add/s.ha.d,
         sfc.add2=fc.se(zg=FSG, cl=HT.D/2, fcl=cfc.d))
...
s.ha.dth <- filter(sh.th0, s.ha.d != 2600) %>% 
  summarize(mean(s.ha.d)) %>% pull()  # fix s.ha.d value where no data; used to be 2600, but that was including p16

sh.th0$s.ha.d[2:3] <- s.ha.dth
...
sh.th <- sh.th0 %>% 
  mutate(cfc.d.stem=cfc.dstem, #use cfc in ctrl plots to estimate cfc in th plots
         ...
         c.l= HT.D * 0.75,  #assume CR of 0.5;
         sfc.add=fc.se(zg=FSG, cl=c.l, fcl=cfc.d),
         sfc.add.stem=sfc.add/s.ha.d,
         sfc.add2=fc.se(zg=FSG, cl=HT.D/2, fcl=cfc.d)) 
...
Final estimates of M ranged from 1.4 to 8.9, as shown in the table
below:
